...
fan_diagn_report    = '/run/hive/fan_controller_report_req'        # репорт диагностики
...
def communication_hive(id_rig_in_server, key_slave, mod_option_hive, const_rpm, rpmfun,rigRpmFanMaximum, option2, terget_temp_min,terget_temp_max, min_fan_rpm, target_mem_temp, selected_mod, device_name):
    if os.path.exists(hive_dir) == False:
        send_mess(str(device_name)+' -connected to Hive OS, "brief" monitoring is also available in the Hive interface', id_rig_in_server)                                                                                                 
        file_info = open(hive_dir, "w+")                                                                                              
        file_info.write(str(device_name))                                                                                                       
        file_info.close()     
    ...
    if mod_option_hive == 1:
        ...
        if os.path.exists(req_recallibrate) == True:
            ...
            file_info.write('device_name: '+ str(device_name) + '\n' + str(res_test_fan))                                                                                                       
            file_info.close() 

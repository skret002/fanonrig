...
def active_cool_mod():                                                                                                                               
    ...
    if ((int(hot_gpu) >= int(terget_temp_min)+1) and (int(hot_gpu) < int(critical_temp))) or int(mem_t) >= (int(target_mem_temp) -2):
        ...
        if (int(hot_gpu) >= int(terget_temp_max) -2) or (int(mem_t) >= 110):   
            ...
            if (optimum_on == 0) and (stable_temp_round <= 15) and (int(mem_t) <110):
                ...
                if  (int(mem_t) >= (int(target_mem_temp) + 3)) or (int(optimum_temp) == int(hot_gpu)) or (int(last_rpm) <= int(real_min_fan_rpm)):
                ...
                get_temp()
                time.sleep(29)
                if int(hot_gpu) >= (int(optimum_temp) + 2)  or int(hot_gpu)< (old_hot_gpu-1) or int(mem_t) >= (int(target_mem_temp) + 3):            

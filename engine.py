...
def active_cool_mod():                                                                                                                               
    ...
    if (int(hot_gpu) >= int(terget_temp_min)) and (int(hot_gpu) < int(critical_temp)):
        ...
        if (int(hot_gpu) >= int(terget_temp_min) + int(int(terget_temp_max - terget_temp_min)/2) + 4) or (int(mem_t) >= 95):   
            ...
            if (optimum_on == 0) and (stable_temp_round <= 5) and (int(mem_t) <95):
                if int(old_hot_gpu) != int(hot_gpu):
                    ...
                    send_mess("усредненый оставляю как есть", id_rig_in_server)                                                                                         
                    get_temp()                                                                                                                       
                    stable_temp_round = stable_temp_round + 1                                                                                        
                    old_hot_gpu = hot_gpu
                    time.sleep(20)
                ...
                send_mess("temp_gpu_freeze "+ temp_gpu_freeze, id_rig_in_server)
                if int(hot_gpu) - int(temp_gpu_freeze) > 0 or int(mem_t) > 86 or optimum_round > 10:
                    send_mess("оптимум - коректирую ", id_rig_in_server)
                    send_mess("опт-кор тригер " + str((int(hot_gpu) - int(temp_gpu_freeze) > 0))+ ' | '+ str((int(mem_t) > 86)) +' | '+ str((optimum_round > 10)),  id_rig_in_server)
                    stab_balance =  int((int(const_rpm) / 100) * ((int(hot_gpu) - int(temp_gpu_freeze))*3)) 
            ...
            elif (stable_temp_round > 5) and (optimum_on == 0) and (int(mem_t) <= 90):  
                get_temp()                                                                     
                ...
                if  int(mem_t) >= 88 or int(optimum_temp) == int(hot_gpu):
                    ...
                    send_mess("ОПТИМУМ ГОТОВ  " + str(last_rpm) , id_rig_in_server)
                    send_mess("ОПТИМУМ ГОТОВ тригер  " + str(int(mem_t) >= 88)+str(int(optimum_temp) == int(hot_gpu)) , id_rig_in_server)
                    old_hot_gpu = hot_gpu

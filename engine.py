...
def active_cool_mod():                                                                                                                               
    ...
    if ((int(hot_gpu) >= int(terget_temp_min)+1) and (int(hot_gpu) < int(critical_temp))) or int(mem_t) >= int(target_mem_temp):
        ...
        if (int(hot_gpu) >= int(terget_temp_max) -2) or (int(mem_t) >= 110):   
            ...
            if (optimum_on == 0) and (stable_temp_round <= 30) and (int(mem_t) <110):
            ...
            elif (stable_temp_round > 30) and (optimum_on == 0) and (int(mem_t) <= (int(target_mem_temp) + 2)):  
...
def search_min_fan_rpm_now(static_option = None):
    ...
    try:                                                                                                                                                                                               
        with open('settings.json', "r+") as file:
            data = json.load(file)                                                                                                                                                                     
            if data["minf"+str(min_fan_rpm_persent)] and  static_option == None:                                                                                                                       
                set_ok = 1                                                                                                                                                                             
        ...
        for i in range(1, int(const_rpm)):                                                                                                                                                                     
            ...
            if (int(rpm) >= int(mr)):
                ...
                with open('settings.json', "r+") as file:                                                                                                                                          
                    data["minf"+str(min_fan_rpm_persent)] = int(give_rpm)   # записываем в файл                                                                                                    
                    data["minf_rpm"+str(min_fan_rpm_persent)] = int(rpm)                                                                                                                           
                    file.seek(0)                                                                                                                                                                   

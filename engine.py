...
def active_cool_mod():                                                                                                                               
    ...
    if (int(hot_gpu) >= int(terget_temp_min)+1) and (int(hot_gpu) < int(critical_temp)):
        ...
        if (int(hot_gpu) >= int(terget_temp_max) -2) or (int(mem_t) >= 110):   
            ...
            if (optimum_on == 0) and (stable_temp_round <= 10) and (int(mem_t) <110):
                ...
                if  (int(mem_t) >= 88) or (int(optimum_temp) == int(hot_gpu)) or (int(last_rpm) == 1) or (int(last_rpm) < int(real_min_fan_rpm)):
                    print('optTRIGER', int(mem_t) >= int(target_mem_temp) , int(optimum_temp) == int(hot_gpu), (int(last_rpm) == 1),(int(last_rpm) < int(real_min_fan_rpm)))                                                             
                    optimum_on = 1                                                                                                                   
...
def search_min_fan_rpm_now(static_option = None):
    ...
    process = subprocess.Popen(mstop.split(), stdout=subprocess.PIPE, stdin=subprocess.PIPE)
    time.sleep(2)
    set_ok = 0                                                                                                                                                                                                                                                                                                                   
    ...
    process = subprocess.Popen(mstart.split(), stdout=subprocess.PIPE, stdin=subprocess.PIPE)
    time.sleep(2)
    process = subprocess.Popen(mstart.split(), stdout=subprocess.PIPE, stdin=subprocess.PIPE)
